...
namespace LinguaBender.Controls
{
    public partial class InfoCard : UserControl
    {
        ...
        public string Persentage
        {
            get { return (string)GetValue(PersentageProperty); }
            set { SetValue(PersentageProperty, value); }
        }

        public static readonly DependencyProperty PersentageProperty = DependencyProperty.Register("Persentage", typeof(string), typeof(InfoCard), 
           new FrameworkPropertyMetadata("0", FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, null, CoercePersentage), ValidatePersentage );
        private static bool ValidatePersentage(object value)
        {
            if (int.TryParse((string)value, out int percentage))
            {
                return percentage >=0 && percentage <= 100;  
            }
            return false;
        }
        private static object CoercePersentage(DependencyObject d, object baseValue)
        {
            if (int.TryParse((string)baseValue, out int percentage))
            {
                return Math.Min(Math.Max(percentage, 0), 100).ToString();
            }
            return "0";
        }
        ...
        public static readonly DependencyProperty IconProperty = DependencyProperty.Register("Icon", typeof(string), typeof(InfoCard));
