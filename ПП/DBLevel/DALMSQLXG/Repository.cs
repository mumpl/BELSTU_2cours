...
namespace DALMSQLXG
{
    ...
    {
        private readonly DALMSQLXGContext _context; //объект для доступа к бд
        private bool _disposed = false; //флаг, который отслеживает, был ли освобожден ресурс

        public Repository(DALMSQLXGContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context)); //Если context == null, выбрасывает исключение
        }
